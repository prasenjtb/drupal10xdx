...
final class MenuApiResource extends ResourceBase {
  ...
  public function __construct(
    ...
    $this->storage = $keyValueFactory->get('news_menu_api');
  }
   ...
   */
  public function get($menu_name): ResourceResponse {  //id
  ...
  private function buildTree(array $tree) {
    ...
    foreach ($tree as $element) {
      //$item = $element->link->getTitle();
      $menu_link = $element->link;

      $item = [
          'title' => $menu_link->getTitle(),
          'url' => $menu_link->getUrlObject()->toString(),
          //'children' => $this->buildTree($element->subtree),
        ];

      if ($element->hasChildren) {
        //$item['children'] = $this->buildTree($element->subtree);         
      }
